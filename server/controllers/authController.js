...
exports.register = async (req, res) => {
  try {
    const { displayName, username, email, password } = req.body; // Destructure user data from request body
    ...
    const user = new User({ displayName, username, email, password }); // Create a new user instance
    await user.save(); // Save the new user to the database
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '1d' }); // Create a JWT token
    res.status(201).json({ token, user: { id: user._id, displayName: user.displayName, username: user.username, email: user.email } }); // Respond with token and user data
  } catch (error) {
...
exports.login = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '1d' }); // Create a JWT token
    res.json({ token, user: { id: user._id, displayName: user.displayName, username: user.username, email: user.email } }); // Respond with token and user data
  } catch (error) {
